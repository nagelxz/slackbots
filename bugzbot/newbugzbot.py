...
class Bugz(object):
	...
	def returnUser(_semail):
		user = fb.viewPerson(semail=semail)
	...
	def assignTicket(ticket_num, user, orig_user, ):
		
		touser = returnUser(user[2])
		fromuser = returnUser(orig_user[2])
		
		if touser and fromuser not "User not found":
	...
	def returnTicket(self, option, ticket_num):
		if option:
			...
			if retrieve_ticket.cases['count'] == str(1):
				...
				print "Something went wrong! Either Fogbugz is down, you're a magician calling a number that does not exist yet, or it somehow returned more than one ticket."
			...
			retrieve_ticket = self.fb.search(q=ticket_num, cols="ixBug,sTitle,sLatestTextSummary,sArea,sProject")
	...
	def processMessage(self, message, ticket_num, orig_user, channel):
		...
		if self.options[1]: #sendto
			...
			try:
				...
				sc.api_call('chat.postMessage', channel=channel, as_user=True, text="Gosh darnit! You screwed it up again.\n\n Make sure you mention a user so I know where to send this thing.")	
